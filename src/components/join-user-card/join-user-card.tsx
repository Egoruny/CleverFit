...
return (
        ...
        className={style.user_card} bodyStyle={{ padding: '4px 16px 0 16px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div className={style.user_info}>
                <AvatarUser isUserCard={true} searchValue={searchValue} index={index}  alt={user.name} src={user.imageSrc} name={name}  surname={surname} backgroundColor={' #F0F5FF'}/>
            ...
            <div className={style.user_trenings}>
               ...
               <Typography.Paragraph style={{color:'#2F54EB'}}> {user.trainingType}</Typography.Paragraph>
                </div>
                <div className={style.trening_type}>
                <Typography.Paragraph type='secondary'> Средняя нагрузка:</Typography.Paragraph>
               <Typography.Paragraph style={{color:'#2F54EB'}}> {user.avgWeightInWeek}кг/нед</Typography.Paragraph>
                </div>
            </div>
            <div className={style.card_footer}>
                <Button block size='small' type='primary' className={style.create_traning_btn} 
                 onClick={() => onChangeHandler(user,user.trainingType,selectetTain,name,surname,user.imageSrc,user.id)}
                >Создать тренировку</Button>
                <div className={style.trening_status}>
                    {user.status &&
                    <>
                     <span>  {isPending && 'ожидает подтверждения'}
                                {isAccepted && 'запрос одобрен'}
                                {isRejected && 'запрос отклонен'} </span> 
